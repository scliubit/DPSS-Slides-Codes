...
%% DICT Configuration
[Psi_DFT,Psi_POL,Psi_DPSS,index2coor] = dict_design(N_UE, N,aperture,k,UE_range_y_min,UE_range_y_max,UE_range_x);
load('../newCE/results/error_HOLO_1000_1.mat')
%% SIM start
epsilon = 0;
% activeRatio=[0.1 0.2 0.25 0.3 0.5 0.75 1];
sampleRatio=[0.25, 0.5, 0.75];
...
%         y_hat = UE.y+(rand()-0.5)*0.25;
%         x_hat = UE.x+(rand()-0.5)*0.25;
...
ylabel('NMSE[dB]')
axis([1 OMP_iternum -20 0])
if ismac
    set(gca,'fontsize',14);
end

if implementations>=1000
    save('../results/NMSE_DFT_dB.mat','NMSE_DFT_dB')
    save('../results/NMSE_POL_dB.mat','NMSE_POL_dB')
    save('../results/NMSE_DPSS_dB.mat','NMSE_DPSS_dB')
end