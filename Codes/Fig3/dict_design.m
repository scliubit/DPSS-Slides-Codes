function [Psi_DFT,Psi_POL,Psi_DPSS,index2coor] = dict_design(N_UE,N,aperture,k,z_min,z_max,x_max)

x_range = linspace(-aperture/2,aperture/2,N);
aperture_UE = aperture/N*N_UE;
% DFT DICT
if N_UE>1
    aR_DFT = dftmtx(N_UE)/sqrt(N_UE);
    aT_DFT = dftmtx(N)/sqrt(N);
    Psi_DFT = kron(conj(aT_DFT),aR_DFT);
else
    Psi_DFT = dftmtx(N)/sqrt(N);
end
% POL DICT
if N_UE>1
    r_inv = linspace(1/z_max,1/z_min,floor(sqrt(N)));%0:0.01:1;
    r = 1./r_inv;
    x = linspace(-x_max,x_max,ceil(sqrt(N)));
    aT_POL = zeros(numel(x_range),numel(r_inv));
    cnt=0;
    ...
    aR_POL = zeros(numel(x_range_R),numel(r_inv));
    cnt=0;
    ...
    Psi_POL = kron(aT_POL,aR_POL)/sqrt(N)/sqrt(N_UE);
else
    r_inv = linspace(1/z_max,1/z_min,floor(sqrt(N)));%0:0.01:1;
    r = 1./r_inv;
    x = linspace(-x_max,x_max,ceil(sqrt(N)));
    Psi_POL = zeros(numel(x_range),numel(r_inv));
    cnt=0;
    index2coor=zeros(numel(r_inv),2);
    for xx =1:numel(x)
        for rr = 1:numel(r)
...
if N_UE>1
    ...
    time_halfbandwidth = 1;%k*aperture/4/pi/z_max;
    % num_seq = 4;
    aR_DPSS = [1];
